...
function getParsers(env) {
  if (parsePromise === null) {
    parsePromise = (async () => {
      ...
      if (!typedefContent) {
        throw new Error("Pre-built typedef.json not found in KV.");
      }
      const modelsContent = await env.ASSETS.get("models.json", "json");
      if (!modelsContent) {
        throw new Error("Pre-built models.json not found in KV.");
      }
...
// .wrangler/tmp/bundle-r89T95/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-r89T95/middleware-loader.entry.ts
