...
import { buildParser } from "../src/server/rsmv/opcode_reader";
import * as commentjson from "comment-json";
import { maprenderConfigSchema } from "../src/server/rsmv/jsonschemas";

async function buildFileTypes() {
	let basedir = path.resolve("./src/server/opcodes");
	...
	for (let file of files) {
		...
		try {
			typesfile += buildParser(null, opcodes as any, typedef as any).getTypescriptType("") + ";\n";
		} catch (e: any) {
			//console.error(e);
			typesfile += "any;\n";
