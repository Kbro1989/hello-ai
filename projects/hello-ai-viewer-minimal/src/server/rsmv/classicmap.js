...
import { classicGroups } from "./classicloader";
import { combineLegacyTexture } from "./legacycache";
import { crc32 } from "./libs/crc32util";
import { HSL2packHSL, RGB2HSL, Stream } from "./utils";
import { constModelsIds } from "./modeltothree";
const chunkSize = 48;
...
export async function getClassicMapData(engine, rs2x, rs2z) {
...
export function classicModifyTileGrid(grid) {
            ...
            for (let x = grid.xsize - 1; x >= 1; x--) {
                ...
                if (tile && (overlay?.type.autoconnect || overlay?.type.indoors)) {
                    ...
                    if (hastop && hasleft && !hasbot && !hasright) {
                        tile.shape = tileshapes[5];
                    }
                    if (hastop && !hasleft && !hasbot && hasright) {
                        tile.shape = tileshapes[6];
                    }
                    if (!hastop && !hasleft && hasbot && hasright) {
                        tile.shape = tileshapes[7];
                    }
                    if (!hastop && hasleft && hasbot && !hasright) {
                        tile.shape = tileshapes[4];
                    }
                }
            }
        }
...
export function classicDecodeMaterialInt(int) {
    let material = 0;
    let invisible = false;
...
export function getClassicLoc(engine, id) {
    const config = engine.classicData;
    let locdata = {};
...
export function classicIntsToModelMods(...matints) {
    let r = {
        color_replacements: [],
...
export async function classicOverlays(engine) {
