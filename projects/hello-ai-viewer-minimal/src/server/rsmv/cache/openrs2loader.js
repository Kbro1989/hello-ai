...
export function loadOpenrsCachelist() {
    cachelist ?? (cachelist = fetch(`${endpoint}/caches.json`).then(q => q.json()));
    return cachelist;
}
export function validOpenrs2Caches() {
    validcachelist ?? (validcachelist = (async () => {
...
export function openrs2GetEffectiveBuildnr(cachemeta) {
...
export class Openrs2CacheSource extends cache.DirectCacheFileSource {
    ...
    async getCacheIndex(major) {
        if (this.buildnr <= 700 && !this.xteaKeysLoaded && major == cacheMajors.mapsquares) {
            this.xteakeysPromise ?? (this.xteakeysPromise = (async () => {
                this.xteakeys ?? (this.xteakeys = new Map());
                let keys = await fetch(`${endpoint}/caches/runescape/${this.meta.id}/keys.json`).then(q => q.json());
                for (let key of keys) {
    ...
    async downloadFile(major, minor) {
        ...
        if (Math.floor(downloadedBytes / 10000000) != Math.floor((downloadedBytes + buf.byteLength) / 10000000)) {
            console.info(`loaded ${(downloadedBytes + res.byteLength) / 1000000 | 0} mb from openrs2`);
        }
