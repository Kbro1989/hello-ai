...
import oldmodels from "generated-opcodes/oldmodels";
...
type OldTextureMapping = {
    ...
    vertexmin: Vector3,
    vertexmax: Vector3,
    args: typeof oldmodels["texflags"][number]
}
...
export function parseRT5Model(modelfile: Buffer, source: EngineCache) {
    ...
    let materialbuffer: Uint16Array | null = null;
    if (modeldata.material) {
      const v = toArrayBufferView(modeldata.material);
      materialbuffer = new Uint16Array(v.buffer, v.byteOffset, Math.floor(v.byteLength / 2));
    }
    ...
    for (let i = 0; i < modeldata.facecount; i++) {
        ...
        if (materialbuffer && materialbuffer[i] == 0 && !colorbuffer || colorbuffer[i] == 0) { continue; }//TODO is this now obsolete?
        let matargument = (materialbuffer ? materialbuffer[i] : 0);
        let submesh = matmesh.get(matargument)!;
        if (!submesh) { throw new Error("material " + matargument + " not found"); }
        ...
        //if (!isFinite(vnormal.x) || !isFinite(vnormal.y) || !isFinite(vnormal.z)) { hadbadnormal = true; }
        ...
        if (colorbuffer) {
            ...
            hadbadcol = hadbadcol || isNaN(r) || isNaN(g) || isNaN(b);
            if (!modeldata.alpha) {
        ...
        if (matargument) {
            ...
            if (mapid == 0) {
                ...
                if (mapping.args.type == 0) {
                    hadbaduv = hadbaduv || isNaN(v0.x) || isNaN(v0.y);
