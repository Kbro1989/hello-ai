...
build_crc_tables();
export function crc32(buf, crc = 0, rangeStart = 0, rangeEnd = buf.length) {
    crc = crc ^ 0xffffffff;
...
function writeUint32BE(arr, value, offset) {
...
export function crc32addInt(int, crc) {
 ...
 */
export function crc32Interlaced(buf, offset, stride, bytes, chunkcount, crc = 0) {
    crc = crc ^ 0xffffffff;
...
export function crc32_backward(buf, crc, rangeStart = 0, rangeEnd = buf.length) {
    crc = crc ^ 0xffffffff;
...
export function intbuffer(value, bigendian = false, bytes = 4) {
...
export function forge_crcbytes(frontcrc, backcrc) {
    let fwd_bytes = intbuffer(frontcrc);
    let bkd_crc = crc32_backward(fwd_bytes, backcrc);
    return intbuffer(bkd_crc);
}
export function forge(str, wanted_crc, pos = str.length, insert = true) {
