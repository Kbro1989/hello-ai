...
import { parseOb3Model } from './rt7model';
import { parseRT5Model } from "./rt5model";
import { materialCacheKey } from "./jmat";
import * as THREE from "three";
import { mappedFileIds, oldConfigMaps } from "./cache/index";
// import { MyCachingFileSource } from "./mycachingfilesource"; // Commented out
import { Bone, BufferAttribute, Object3D, Skeleton } from "three";
import { parse } from "./opdecoder";
import { crc32, CrcBuilder } from "./libs/crc32util";
import { makeImageData } from "./rsmv/imgutils";
import { ParsedTexture, parseSprite } from "./rsmv/3d/sprite";
import { combineLegacyTexture, legacyGroups, legacyMajors, legacyPreload, parseLegacyImageFile } from "./legacycache";
import { classicConfig, ClassicFileSource, classicGroups } from "./classicloader";
import { parseRT2Model } from "./rsmv/3d/rt2model";
import { classicRoof14, classicRoof16, classicRoof13, classicRoof10, classicRoof17, classicRoof12, materialPreviewCube, classicWall, classicWallDiag, classicRoof15, getAttributeBackingStore } from "./rsmv/3d/modelutils";
import { classicOverlays, classicUnderlays } from "./classicmap";
import { loadProcTexture } from "./rsmv/3d/proceduraltexture";
import { minimapLocMaterial } from "./rsmv/rs3shaders";
import { getDependencies } from "./rsmv/scripts/dependencies";
...
export function augmentZOffsetMaterial(mat, zoffset) {
...
export function augmentThreeJsFloorMaterial(mat, isminimap) {
...
import { CachingFileSource } from "./memorycache";
export class EngineCache extends CachingFileSource {
    ...
    async getDependencyGraph() {
        this.dependencyGraph ?? (this.dependencyGraph = getDependencies(this, { lazyMapChunks: true }));
...
export async function* iterateConfigFiles(cache, major) {
    if (cache.legacyData) {
        let files = null;
...
export async function detectTextureMode(source) {
...
async function convertMaterialToThree(source, material, hasVertexAlpha, minimapVariant) {
    ...
    if (typeof material.textures.diffuse != "undefined" && source.textureType != "none") {
        ...
        if (material.textures.normal) {
            ...
            for (let i = 0; i < data.length; i += 4) {
                ...
                normals.data[i + 1] = data[i + 1];
                normals.data[i + 2] = data[i + 3];
                normals.data[i + 0] = (Math.sqrt(Math.max(1 - dx * dx - dy * dy, 0)) + 1) * 127.5;
    ...
    if (material.uvAnim) {
        ((_a = mat.userData).gltfExtensions ?? (_a.gltfExtensions = {})).RA_materials_uvanim = {
...
export class ThreejsSceneCache {
    constructor(scenecache, modeltype) {
        ...
        this.threejsTextureCache = new Map();
        this.threejsMaterialCache = new Map();
...
ThreejsSceneCache.textureIndices = {
    diffuse: {
...
export function applyMaterial(mesh, parsedmat, minimapVariant, inplace = false) {
 ...
 */
export function mergeBoneids(model) {
...
export function mergeModelDatas(models) {
    let r = {
...
export async function ob3ModelToThree(scene, model) {
...
export function getModelHashes(model, id) {
    let meshhashes = [];
