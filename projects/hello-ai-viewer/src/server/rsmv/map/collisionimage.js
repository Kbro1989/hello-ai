...
export function drawCollision(grids, rect, maplevel, pxpertile, wallpx) {
    ...
    let colcheck = (tiles, index, lowx, lowz, highx, highz) => {
        ...
        for (let tile of tiles) {
            walk || (walk = !!tile.effectiveCollision && tile.effectiveCollision.walk[index]);
            sight || (sight = !!tile.effectiveCollision && tile.effectiveCollision.sight[index]);
        }
        if (walk) {
            ctx.fillStyle = (sight ? wallcol : walkcol);
            ctx.fillRect(tiles[0].x / tiledimensions * pxpertile + (lowx ? 0 : pxpertile - wallpx), tiles[0].z / tiledimensions * pxpertile + (lowz ? 0 : pxpertile - wallpx), (lowx && highx ? pxpertile : wallpx), (lowz && highz ? pxpertile : wallpx));
        }
    };
