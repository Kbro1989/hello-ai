...
import { naiveDirname } from "../scriptrunner";
import { assertSchema, maprenderConfigSchema } from "../jsonschemas";
import * as commentjson from "comment-json";
export function parseMapConfig(configfile) {
...
export class MapRender {
    constructor(config) {
        this.version = 0;
        this.workerid = "default";
        //optional api's when rendering history stuff
        this.rendermetaLayer = undefined;
        this.config = config;
    }
...
export class MapRenderFsBacked extends MapRender {
    ...
    async getFileResponse(name, version) {
        ...
        try {
            ...
            return new Response(toBlobPart(file), { headers: { "content-type": mimetype } });
        }
        catch {
...
export class MapRenderDatabaseBacked extends MapRender {
    constructor(endpoint, auth, workerid, uploadmapid, config, rendermetaLayer, overwrite, ignorebefore) {
        super(config);
        this.postThrottler = new FetchThrottler(20);
        this.fileThrottler = new FetchThrottler(20);
    ...
    static async create(endpoint, auth, uploadmapid, overwrite, ignorebefore) {
        ...
        localStorage.map_workerid ?? (localStorage.map_workerid = workerid);
    ...
    async symlinkBatch(files) {
        ...
        let send = await this.postThrottler.apiRequest(`${this.endpoint}/uploadbatch?mapid=${this.uploadmapid}`, {
            ...
            body: toBlobPart(JSON.stringify(files))
        });
        if (!send.ok) {
