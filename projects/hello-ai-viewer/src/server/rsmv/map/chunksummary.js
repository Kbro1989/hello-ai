...
import { getTileHeight, rs2ChunkSize, tiledimensions, transformVertexPositions } from "../3d/mapsquare";
import { ob3ModelToThree } from "../3d/modeltothree";
import { ModelBuilder } from "../3d/modelutils";
import { crc32addInt } from "../libs/crc32util";
import { getOrInsert } from "../utils";
export function getLocImageHash(grid, info) {
...
export function chunkSummary(grid, locdefs, rect) {
    let locids = new Map();
    let locs = [];
    let hashes = new Map();
...
export function mapsquareFloorDependencies(grid, deps, chunk) {
    let groups = [];
    const groupsize = 2;
...
export function mapsquareLocDependencies(grid, deps, locs, chunkx, chunkz) {
...
export function compareFloorDependencies(tilesa, tilesb, levela, levelb) {
    let vertsets = [];
...
export function compareLocDependencies(chunka, chunkb, levela, levelb) {
    let vertsets = [];
    let iloca = 0, ilocb = 0;
    while (true) {
...
export async function mapdiffmesh(scene, points, col = [255, 0, 0]) {
...
export async function generateLocationHashBoxes(scene, locs, grid, chunkx, chunkz, level) {
...
export async function generateFloorHashBoxes(scene, grid, chunk, level) {
    let deps = await scene.engine.getDependencyGraph();
...
export function pointsIntersectProjection(projection, points) {
    ...
    calculateDiffArea(imgwidth, imgheight) {
        ...
        for (let y = 0; y < nmeans; y++) {
            for (let x = 0; x < nmeans; x++) {
                let center = new Vector2((x + 0.5) / nmeans * gridsize, (y + 0.5) / nmeans * gridsize);
                buckets.push({
                    center: center,
...
globalThis.test = async (chunka, levela, levelb = 0, chunkb = chunka) => {
    ...
    return () => {
        ...
        let modelmatrix = new Matrix4().makeTranslation(chunka.chunkx * tiledimensions * chunka.loaded.chunkSize, 0, chunka.chunkz * tiledimensions * chunka.loaded.chunkSize).premultiply(chunka.rootnode.matrixWorld);
