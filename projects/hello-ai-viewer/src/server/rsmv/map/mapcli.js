...
import { MapRenderDatabaseBacked, MapRenderFsBacked, parseMapConfig } from "./backends";
...
import { ClassicFileSource, detectClassicVersions } from "../cache/classicloader";
...
let cmd = cmdts.command({
    ...
    handler: async (args) => {
        ...
        if (!args.builds) {
            ...
            let classicIterator = async function* (ascending) {
            ...
            let rs2Iterator = async function* (ascending) {
            ...
            let cacheiterator = async function* (ascending) {
                if (ascending) {
