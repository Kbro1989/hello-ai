...
export interface RSModel {
  rootnode?: THREE.Object3D;              // used for position.set
  addToScene?(renderer: ThreeJsRenderer): void; // optional, called in scenenodes
  cleanup?(): void;                       // called in prom.then(models => models.forEach(q => q.cleanup()))
  // If there are other dynamic members used, add as optional
  [key: string]: any;
}

// RSMapChunk interface
export interface RSMapChunk {
  chunkx: number;                         // used extensively in scenenodes.tsx
  chunkz: number;
  loaded?: {
    sky?: any;                            // type inferred from skysettings
  };
  renderSvg?(arg1: number, arg2: boolean): Promise<any>; // used in renderSvg call
  setToggles?(toggles: any, visible: boolean): void;   // called in q.setToggles(...)
  chunkdata?: any;                        // awaited in chunkdata access
  cleanup?(): void;                        // q.models.forEach(q => q.cleanup())
  // placeholder for other dynamic members
  [key: string]: any;
}

// RSMapChunkGroup interface
export interface RSMapChunkGroup {
  models: Map<any, RSMapChunk | RSModel>; // used in chunkgroups.forEach(q => q.models...)
}
...
export class RSModel extends TypedEmitter<{ loaded: undefined, animchanged: number }> implements ThreeJsSceneElementSource, RSModel {
	...
	setAnimation(animId: number) {
		...
		if (typeof animId === "number" && this.animations?.[animId]) {
...
export class RSMapChunkGroup extends TypedEmitter<{ loaded: undefined, changed: undefined }> implements ThreeJsSceneElementSource, RSMapChunkGroup {
	chunks: RSMapChunk[];
	rootnode = new THREE.Group();
	renderscene: ThreeJsRenderer | null = null;
	mixer = new AnimationMixer(this.rootnode);
...
export class RSMapChunk extends TypedEmitter<{ loaded: RSMapChunkData, changed: undefined }> implements ThreeJsSceneElementSource, RSMapChunk {
