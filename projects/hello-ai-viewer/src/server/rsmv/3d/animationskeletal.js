...
import { AnimationClip, Bone, Euler, Matrix4, Quaternion, QuaternionKeyframeTrack, Skeleton, SkinnedMesh, VectorKeyframeTrack } from "three";
//new anims
//115416  obelisk
//114652  pof totem
...
export async function mountSkeletalSkeleton(rootnode, cache, framebaseid) {
...
export async function parseSkeletalAnimation(cache, animid) {
    ...
    for (let index = 0; index < animtracks.length;) {
        ...
        if (tracktype.t == "translate") {
            convertedtracks.push(new VectorKeyframeTrack(`${bonename}.position`, times, data));
        }
        if (tracktype.t == "scale") {
            ...
            convertedtracks.push(new VectorKeyframeTrack(`${bonename}.scale`, times, data));
        }
        if (tracktype.t == "rotate") {
            ...
            convertedtracks.push(new QuaternionKeyframeTrack(`${bonename}.quaternion`, times, quatdata));
        }
    }
