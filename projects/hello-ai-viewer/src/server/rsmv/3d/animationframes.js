...
import { AnimationClip, AnimationMixer, Bone, Euler, Matrix4, Quaternion, QuaternionKeyframeTrack, Skeleton, SkinnedMesh, Vector3, VectorKeyframeTrack } from "three";
import { getBoneCenters } from "./rt7model";
export function mountBakedSkeleton(rootnode, model) {
    let centers = getBoneCenters(model);
    let rootbone = new Bone();
    rootnode.add(rootbone);
    let leafbones = [rootbone];
    let rootbones = [];
    let inverses = [new Matrix4()];
...
export async function parseAnimationSequence4(loader, sequenceframes) {
...
export function getFrameClips(framebase, framesparsed) {
