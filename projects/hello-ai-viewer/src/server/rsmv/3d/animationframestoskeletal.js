...
import { AnimationClip, Bone, Matrix4, Quaternion, QuaternionKeyframeTrack, Skeleton, Vector3, VectorKeyframeTrack } from "three";
import { getBoneCenters } from "./rt7model";
import { getFrameClips } from "./animationframes";
function isEqualTr(a, b) {
    return a.type == b.type && a.data == b.data && a.inverse == b.inverse;
}
function isInverseTr(a, b) {
    return a.type == b.type && a.data == b.data && a.inverse != b.inverse;
}
async function parseAnimationSequence3(loader, sequenceframes) {
...
function buildFramebaseSkeleton(framebase) {
    ...
    for (let [index, base] of framebase.data.entries()) {
        if (base.type == 0) {
            if (base.data.length == 0) {
                ...
                if (!pivot) {
                    throw new Error("no shared pivot found");
                }
            }
        }
        actions.push({
    ...
    let buildskeleton = function (bones, transSkip) {
        let children = [];
        //direct children for bones that have no more transforms left
        for (let bone of bones) {
            if (bone.stack.length == transSkip) {
                bones.delete(bone);
                if (bone.boneids.length != 1) {
                    throw new Error("single owner bone expected");
                }
                children.push({
                    bakedTransform: null,
                    translateconst: [],
        ...
        if (transSkip == 0) {
            for (let bone of bones) {
                ...
                if (index == -1) {
                    throw new Error("bake not found");
                }
                let bakedstack = bone.stack.slice(index);
                let friends = new Set();
                boneloop: for (let bone of bones) {
                    if (bone.lastbaked != bake) {
                        continue;
                    }
                    if (bone.stack.length < bakedstack.length) {
                        continue;
                    }
        ...
        while (bones.size != 0) {
            ...
            digloop: for (; !ownerbone; newskip++) {
                ...
                for (let friend of friends) {
                    ...
                    if (!tr) {
                        ...
                        if (resultbone.bakedTransform) {
                            break digloop;
                        }
                        if (tr.type == "translate" && (resultbone.rotate.length != 0 || resultbone.scale.length != 0)) {
                            break digloop;
                        }
                        if (tr.type == "translateconst" && (resultbone.rotate.length != 0 || resultbone.scale.length != 0)) {
                            break digloop;
                        }
                        if (tr.type == "rotate" && resultbone.scale.length != 0) {
                            break digloop;
                        }
                    }
                    else {
                ...
                if (tr) {
                    if (tr.type == "translateconst") {
                        resultbone.translateconst.push(tr);
                    }
                    if (tr.type == "translate") {
                        resultbone.translate.push(tr);
                    }
                    if (tr.type == "rotate") {
                        resultbone.rotate.push(tr);
                    }
                    if (tr.type == "scale") {
                        resultbone.scale.push(tr);
                    }
                    if (tr.type != "translateconst") {
                        hadnonconst = true;
                    }
            ...
            if (ownerbone) {
                if (ownerbone.boneids.length != 1) {
                    throw new Error("ownerbone with id length 1 expected");
                }
    ...
    let logboneinit = (bone, indent) => {
        //TODO i don't think this .find is correct
        let actionmap = (tr) => tr.map(q => actions.find(w => w.index == q.data)?.index + (q.inverse ? "!" : ""));
...
function findSharedPivot(bones) {
...
function mountAnimation(rootboneinits, clips, frametimes, bonecenters) {
    let nframes = frametimes.length;
    let keyframetracks = [];
    let extrabonecounter = 0;
    let indexedbones = [];
    let missingpivots = 0;
    function iter(init, parentbonestates) {
        ...
        for (let tr of init.translateconst) {
            ...
            if (totalweight != 0) {
                bone.position.set(bone.position.x + xsum / totalweight, bone.position.y + ysum / totalweight, bone.position.z + zsum / totalweight);
            }
            else {
                missingpivots++;
            }
        ...
        if (translate) {
            keyframetracks.push(new VectorKeyframeTrack(`${bone.name}.position`, frametimes, translate));
        }
        if (rotate) {
            keyframetracks.push(new QuaternionKeyframeTrack(`${bone.name}.quaternion`, frametimes, rotate));
        }
        if (scale) {
            keyframetracks.push(new VectorKeyframeTrack(`${bone.name}.scale`, frametimes, scale));
        }
        return bone;
    }
...
function bakeTransformStack(stack, clips, frametimes, bonecenters) {
    let nframes = frametimes.length;
    ...
    for (let stacki = stack.length - 1; stacki >= 0; stacki--) {
        ...
        for (let i = 0; i < nframes; i++) {
            ...
            if (action.type == "translateconst") {
                ...
                transform.makeTranslation((totalweight == 0 ? 0 : xsum / totalweight), (totalweight == 0 ? 0 : ysum / totalweight), (totalweight == 0 ? 0 : zsum / totalweight));
                matrix.multiply(transform);
            }
