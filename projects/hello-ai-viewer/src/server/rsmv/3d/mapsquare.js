import { packedHSL2HSL, HSL2RGB, posmod, getOrInsert } from "../utils";
import { cacheConfigPages, cacheMajors, cacheMapFiles, lastClassicBuildnr, lastLegacyBuildnr } from "../constants";
import { parse } from "../opdecoder";
import { augmentThreeJsFloorMaterial, ob3ModelToThree, applyMaterial, augmentZOffsetMaterial } from "./modeltothree";
import { BufferAttribute, Object3D, Quaternion, Vector3 } from "three";
import { defaultMaterial, materialCacheKey } from "./jmat";
...
export function mapRectsIntersect(a, b) {
...
export function mapRectContains(rect, x, z) {
...
export class TileProps {
    ...
    constructor(height, tilesettings, tilex, tilez, level, docollision) {
        ...
        this.debug_raw = null;
        this.rawOverlay = undefined;
        this.rawUnderlay = undefined;
        this.next01 = undefined;
        this.next10 = undefined;
        this.next11 = undefined;
        this.shape = defaulttileshape;
        this.underlayVisible = false;
        this.overlayVisible = false; //these should probably be merged
        this.normalX = 0;
        this.normalZ = 0;
        this.bleedsOverlayMaterial = false;
        this.originalUnderlayColor = defaultVertexProp.color;
        this.rawCollision = undefined;
        this.effectiveCollision = undefined;
...
export function modifyMesh(mesh, mods) {
...
export function getMorphMatrix(morph, gridoffsetx, gridoffsetz) {
...
export function transformVertexPositions(pos, morph, grid, modelheight, gridoffsetx, gridoffsetz, newpos, newposindex = 0, inputstart = 0, inputend = pos.count) {
    ...
    newpos ?? (newpos = new THREE.BufferAttribute(new Float32Array(inputcount * 3), 3));
...
export function getTileHeight(grid, x, z, level) {
        ...
        for (let z = this.zoffset; z < this.zoffset + this.zsize; z++) {
            for (let x = this.xoffset; x < this.xoffset + this.xsize; x++) {
                ...
                for (let level = 0; level < this.levels; level++) {
                    ...
                    let dydz = 0;
                    if (tile_w && tile_e) {
                        dydx = (tile_e.y - tile_w.y) / (2 * tiledimensions);
                    }
                    if (tile_s && tile_n) {
                        dydz = (tile_n.y - tile_s.y) / (2 * tiledimensions);
                    }
...
export async function getMapsquareData(engine, chunkx, chunkz) {
...
export async function parseMapsquare(engine, chunkx, chunkz, opts) {
...
export async function mapsquareSkybox(scene, mainchunk) {
...
export async function mapsquareFloors(scene, grid, chunk, opts) {
    let floors = [];
...
export async function renderMapSquare(cache, parsedsquare, chunkx, chunkz, opts) {
...
class SimpleTexturePacker {
    constructor(width, height) {
        this.padsize = 32; //was still bleeding at 16
        this.allocs = [];
        this.map = new Map();
        this.allocx = 0;
        this.allocy = 0;
    ...
    convertToThreeTexture() {
        return this.resultSource ?? (this.resultSource = (() => {
        ...
        for (let alloc of this.allocs) {
            ...
            for (let y = yy1; y < yy2; y = nexty) {
                ...
                for (let x = xx1; x < xx2; x = nextx) {
                    ...
                    drawSubimg(alloc.img, alloc.x + x, alloc.y + y, posmod(x, alloc.img.width), posmod(y, alloc.img.height), nextx - x, nexty - y);
                }
            }
        }
        this.result = cnv;
...
export function defaultMorphId(locmeta) {
    let newid = -1;
    if (locmeta.morphs_1) {
...
export async function resolveMorphedObject(source, id) {
    let resolvedid = id;
...
export async function mapsquareOverlays(engine, grid, locs) {
...
export function mapsquareObjectModels(cache, inst, minimap = false) {
    let locmodels = [];
    ...
    let translate = new Vector3((objectmeta.translateX ?? 0) * translatefactor, -(objectmeta.translateY ?? 0) * translatefactor, //minus y!!!
    (objectmeta.translateZ ?? 0) * translatefactor);
    const scalefactor = 1 / 128; //estimated fit was 127.5 ...
    let scale = new Vector3((objectmeta.scaleX ?? 128) * scalefactor, (objectmeta.scaleY ?? 128) * scalefactor, (objectmeta.scaleZ ?? 128) * scalefactor);
    ...
    if (minimap) {
        translate.y -= 0.2 * tiledimensions;
    }
    if (inst.placement) {
        translate.add(new Vector3().set(inst.placement.translateX ?? 0, -(inst.placement.translateY ?? 0), inst.placement.translateZ ?? 0));
        ...
        if (inst.placement.scaleX || inst.placement.scaleY || inst.placement.scaleZ) {
            scale.multiply(new Vector3().set((inst.placement.scaleX ?? 128) / 128, (inst.placement.scaleY ?? 128) / 128, (inst.placement.scaleZ ?? 128) / 128));
        }
...
export async function mapsquareObjects(engine, grid, locations, originx, originz, collision = false) {
    let locs = [];
...
export async function generateLocationMeshgroups(scene, locbases, minimap = false) {
...
export function meshgroupsToThree(grid, meshgroup, rootx, rootz, material, locrenders) {
