...
import { HSL2packHSL, RGB2HSL } from "../utils";
export function avatarStringToBytes(text) {
    let base64 = text.replace(/\*/g, "+").replace(/-/g, "/");
    return Buffer.from(base64, "base64");
}
export function bytesToAvatarString(buf) {
    let base64 = buf.toString("base64");
    return base64.replace(/\+/g, "*").replace(/\//g, "-").replace(/=/g, "");
}
export function lowname(name) {
...
export async function avatarToModel(engine, buffer, head) {
...
export function writeAvatar(avatar, gender, npc) {
    let base = {
        gender: gender,
        npc: npc,
        player: null
...
export function appearanceUrl(name) {
