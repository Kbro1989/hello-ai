...
import { binaryOpIds, binaryOpSymbols, typeToPrimitive, knownClientScriptOpNames, namedClientScriptOps, variableSources, StackDiff, StackInOut, StackList, dynamicOps, subtypes, subtypeToTs, ExactStack, tsToSubtype, getOpName, makeop, primitiveToUknownExact, StackConstants, longBigIntToJson } from "./definitions";
...
class ParseContext {
    ...
    declareVar(varname, type) {
        if (Object.hasOwn(this.vars, varname)) {
            ...
            if (res.stacktype != typeToPrimitive(type)) {
                throw new Error(`tried to redeclare var ${varname} with incompatible stack type (was: ${res.stacktype} new: ${typeToPrimitive(type)})`);
            }
            if (res.type != type) {
                if (res.type == subtypes.unknown_int) {
                    res.type = type;
                }
                else if (type == subtypes.unknown_int) { /*nop*/ }
                else if (res.type == subtypes.unknown_int) {
                    res.type = type;
                }
                else if (type == subtypes.unknown_int) { /*nop*/ }
                else if (res.type == subtypes.unknown_int) {
                    ...
                    throw new Error(`Tried to redeclare var ${varname} with incompatible subtype (was: ${subtypeToTs(res.type)}, new: ${subtypeToTs(type)})`);
                }
            }
            return res;
        }
...
function scriptContext(ctx) {
        ...
        for (let i = varnames.length - 1; i >= 0; i--) {
            ...
            if (varname == "") {
                ...
                if (typeToPrimitive(vartype) != stacktype) {
                    throw new Error(`type of value and target variable did not match for var ${varname}:${subtypeToTs(vartype)}, ${subtypeToTs(primitiveToUknownExact(stacktype))}`);
                }
    ...
    function* functionStatement() {
        ...
        let res = new ClientScriptFunction(name, new StackList(argtypes.map(q => typeToPrimitive(tsToSubtype(q.type)))), new StackList(returntypes.map(q => typeToPrimitive(tsToSubtype(q)))), new StackDiff());
...
export function parseClientscriptTs(deob, code) {
    let ctx = new ParseContext(deob, null);
    let scope = scriptContext(ctx);
    let res = parse(code, scope.functionStatement());
    return res;
}
//TODO remove
...
export function writeOpcodeFile(calli) {
    let res = "";
...
export function writeClientVarFile(calli) {
    let res = "";
