...
export function dependencyGroup(deptype, id) {
    return (dependentToId(deptype) << 27) | (id << 9);
}
export function dependencyIndex(subtype, index) {
    return (primitiveToId(subtype) << 25) | index;
}
export function knownDependency(fulltype) {
    return (primitiveToId(typeToPrimitive(fulltype)) << 25) | fulltype;
}
export function keyToPrimitive(key) {
...
export function getOpName(id) {
    return knownClientScriptOpNames[id] ?? `unk${id}`;
}
export function longJsonToBigInt(tuple) {
...
export function tsToSubtype(tscode) {
    if (tscode == "cs2bool") {
...
export function typeToPrimitive(typeint) {
...
export function primitiveToUknownExact(stacktype) {
    if (stacktype == "int") {
...
export const knownClientScriptOpNames = {
    ...rs3opnames,
...
export function makeop(opcode, imm = 0, imm_obj = null) {
    return { opcode, imm, imm_obj };
}
export function makejump(label) {
