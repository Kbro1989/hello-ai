import { getArgType, getReturnType } from "./callibrator";
import { debugAst } from "./codewriter";
import { branchInstructions, branchInstructionsOrJump, dynamicOps, typeToPrimitive, namedClientScriptOps, StackDiff, StackInOut, StackList, StackConstants, getParamOps, subtypes, branchInstructionsInt, branchInstructionsLong, ExactStack, typeuuids, makeop } from "./definitions";
...
export class BranchingStatement extends AstNode {
    constructor(opcodeinfo, originalindex) {
        super(originalindex);
        this.knownStackDiff = new StackInOut(new StackList(["int", "int"]), new StackList(["int"])); //TODO not correct, we also use this for longs
        this.op = opcodeinfo;
    }
    getOpcodes(ctx) {
...
export class RawOpcodeNode extends AstNode {
    constructor(index, op, opinfo) {
        super(index);
        this.unknownstack = false; //multiple possible explanations for stack usage
        this.op = op;
        this.opinfo = opinfo;
    }
    getOpcodes(ctx) {
...
export function getNodeStackOut(node) {
...
export function getNodeStackIn(node) {
    if (node.knownStackDiff) {
...
export function translateAst(ast) {
    ...
    for (let i = 0; i < ast.children.length - 3; i++) {
        ...
        if (isNamedOp(pushx, namedClientScriptOps.pushlocalint) &&
    ...
    for (let node = cursor.goToStart(); node; node = cursor.next()) {
        if (node instanceof RawOpcodeNode && (node.op.opcode == namedClientScriptOps.poplocalint ||
...
export function varArgtype(stringconst, lastintconst) {
...
export function setRawOpcodeStackDiff(consts, calli, node) {
    if (branchInstructionsInt.includes(node.opinfo.id)) {
        ...
        if (typeof tablefield == "number") {
            ...
            if (column) {
                node.knownStackDiff = StackInOut.fromExact([subtypes.dbrow, subtypes.int, subtypes.int], (subfield != 0 ? [column.columns[subfield - 1].type] : column.columns.map(q => q.type)));
            }
        }
    }
        ...
        if (typeof outtypeid == "number" && typeof intypeid == "number") {
            node.knownStackDiff = StackInOut.fromExact([subtypes.int, subtypes.int, subtypes.enum, intypeid], [outtypeid]);
        }
    }
        ...
        node.knownStackDiff = new StackInOut(new StackList(Array(node.op.imm).fill("string")), new StackList(["string"]));
    }
        ...
        if (varmeta) {
            ...
            node.knownStackDiff = StackInOut.fromExact((ispop ? value : []), (ispop ? [] : value));
        }
    }
...
function addKnownStackDiff(children, calli) {
    ...
    for (let node of children) {
        ...
        if (node instanceof RawOpcodeNode) {
            ...
            stackinout ?? (stackinout = node.knownStackDiff ?? node.opinfo.stackinfo);
            hasunknown || (hasunknown = node.unknownstack);
        }
...
export function generateAst(calli, script, ops, scriptid) {
...
export function parseClientScriptIm(calli, script, fileid = -1) {
