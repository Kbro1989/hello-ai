...
import { isNamedOp, parseClientScriptIm, RawOpcodeNode, RewriteCursor, FunctionBindNode } from "./ast";
...
(function (implementations) {
    var paramtable = new Map();
    var structatble = new Map();
    var enumtable = new Map();
    ...
    implementations.struct_getparam = struct_getparam;
    ...
    implementations.enum_getvalue = enum_getvalue;
    ...
    implementations.ENUM_GETOUTPUTCOUNT = ENUM_GETOUTPUTCOUNT;
})(implementations || (implementations = {}));
export async function extractClientModuleCode(output, outdir, source, entryscripts) {
...
export function analyzeCallGraph(mod) {
    ...
    let rootnode = { id: -1, parent: null, children: new Set() };
...
function scriptdeps(mod, fn) {
    ...
    for (let node = cursor.goToStart(); node; node = cursor.next()) {
        ...
        if (node instanceof RawOpcodeNode) {
            stackinout ?? (stackinout = node.opinfo.stackinfo);
