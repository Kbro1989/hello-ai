...
import { branchInstructionsOrJump, namedClientScriptOps, StackDiff, getOpName, makeop, pushOrPopLocalOps, makejump, StackList, variableSources } from "./definitions";
...
export class OpcodeWriterContext {
    constructor(calli) {
        ...
        this.subfunctions = new Map();
        this.returntableLabel = null;
        this.returnsites = new Map();
 ...
 */
export function writeRawStackSubFunction(ctx, subfunc) {
    let opdata = [];
...
export function writeSubFunction(ctx, subfunc) {
    let opdata = [];
    let intype = subfunc.func.argtype.getStackdiff();
    let outtype = subfunc.func.returntype.getStackdiff();
...
export function astToImJson(calli, func) {
...
export function mergeScriptJsons(script1, script2, secret) {
