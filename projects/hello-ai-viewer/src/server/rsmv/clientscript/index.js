...
export async function compileClientScript(source, code) {
...
export async function renderClientScript(source, buf, fileid, relativeComps = false, notypes = false, int32casts = false) {
    let calli = await prepareClientScript(source);
...
export async function prepareClientScript(source) {
...
//     })()
// }
export function clientscriptHash(script) {
    let hash = 0;
    ...
    for (let op of script.opcodedata) {
        ...
        if (op.imm_obj == null) { }
            ...
            hash = crc32addInt(op.imm_obj, hash);
        }
        else if (Array.isArray(op.imm_obj)) {
            hash = crc32addInt(op.imm_obj[0], hash);
            hash = crc32addInt(op.imm_obj[1], hash);
        }
        else if (typeof op.imm_obj == "string") {
            hash = crc32(Buffer.from(op.imm_obj, "latin1"), hash);
        }
        else {
