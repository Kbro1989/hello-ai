...
export function cliFsOutputType(ctx, fsname) {
    return {
...
export function cliFsInputType() {
    return {
        async from(str) { return new CLIScriptFS(str); },
...
export function cliApi(ctx) {
    ...
    const testdecode = command({
        ...
        handler: async (args) => {
            ...
            if (!mode) {
                throw new Error(`mode ${args.mode} not found, possible modes: ${Object.keys(cacheFileJsonModes).join(", ")}`);
            }
    ...
    const extract = command({
        ...
        async handler(args) {
            ...
            if (args.keepbuffers) {
                decoderflags.keepbuffers = "true";
            }
            if (args.cs2relativecomps) {
                decoderflags.cs2relativecomps = "true";
            }
            if (args.cs2notypes) {
                decoderflags.cs2notypes = "true";
            }
            if (args.cs2intcasts) {
                decoderflags.cs2intcasts = "true";
            }
    ...
    const filehist = command({
        ...
        async handler(args) {
            let output = ctx.getConsole();
            if (!cacheFileDecodeModes[args.mode]) {
                throw new Error("unkown mode");
            }
            let id = args.id.split(".").map(q => +q);
            if (id.length == 0 || id.some(q => isNaN(q))) {
                throw new Error("invalid id");
            }
            await output.run(fileHistory, args.save, args.mode, id, null, null);
        }
    });
    const edit = command({
        name: "edit",
