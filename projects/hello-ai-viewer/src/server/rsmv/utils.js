export function checkObject(obj, props) {
...
export function cacheFilenameHash(name, oldhash) {
    let hash = 0;
    if (oldhash) {
...
export function stringToMapArea(str) {
...
export function stringToFileRange(str) {
    let parts = str.split(",");
...
export function getOrInsert(map, key, fallback) {
    let val = map.get(key);
    if (val === undefined) {
        val = fallback();
        map.set(key, val);
...
export function rsmarkupToSafeHtml(str) {
    let res = "";
    let tokenstack = [];
    try {
        while (str) {
 ...
 */
export function arrayEnum(v) {
    return v;
}
/**
 * Used to provide literal typing of map keys while also constraining each value
 */
export function constrainedMap() {
    return function (v) {
        return v;
    };
}
export const Stream = function Stream(data, scan = 0) {
...
export function HSL2RGBfloat(hsl) {
    var h = hsl[0];
    var s = hsl[1];
    var l = hsl[2];
    var r, g, b;
...
export function HSL2packHSL(h, s, l) {
    if (h < 0) {
        h += 1;
    }
    return (Math.round(h * 63) << 10) | (Math.round(s * 7) << 7) | (Math.round(l * 127));
}
export function packedHSL2HSL(hsl) {
...
export class CallbackPromise extends Promise {
...
export function trickleTasks(name, parallel, tasks) {
...
export async function trickleTasksTwoStep(parallel, tasks, steptwo) {
    let writecounter = 0;
...
export class WeakRefMap {
    constructor() {
        this.map = new Map();
        this.registry = new FinalizationRegistry(k => this.map.delete(k));
    }
    set(key, value) {
...
export function findParentElement(el, cond, fallback = null) {
    while (el) {
        if (cond(el)) {
            return el;
        }
