import { toBlobPart } from "../utils/buffer-utils";
export function makeImageData(data, width, height) {
    if (!data) {
...
export async function pixelsToImageFile(imgdata, format, quality) {
...
export async function fileToImageData(file, mimetype, stripAlpha) {
    if (typeof ImageDecoder != "undefined") {
        ...
        let img = new Image();
        let blob = new Blob([toBlobPart(file)], { type: mimetype });
...
export async function pixelsToDataUrl(imgdata) {
    if (typeof HTMLCanvasElement != "undefined") {
        let cnv = document.createElement("canvas");
        cnv.width = imgdata.width;
        cnv.height = imgdata.height;
...
export function isImageEqual(overlay, background, x1 = 0, y1 = 0, width = overlay.width, height = overlay.height) {
...
export function maskImage(img, rects) {
    //set all alpha to 0
...
export function isImageEmpty(img, mode, x1 = 0, y1 = 0, width = img.width, height = img.height) {
...
export async function canvasToImageFile(cnv, format, quality) {
...
export function sliceImage(img, bounds) {
...
export function findImageBounds(img) {
    let intview = new Uint32Array(img.data.buffer, img.data.byteOffset, img.data.byteLength / 4);
...
export function dumpTexture(img, flip = false) {
    let cnv = document.createElement("canvas");
    let ctx = cnv.getContext("2d", { willReadFrequently: true });
    if (flip) {
...
export function drawTexture(ctx, img) {
