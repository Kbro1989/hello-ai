...
export async function previewAllFileTypes(output, fs, source, filespermode = 10) {
    ...
    for (let [untypedmodename, mode] of Object.entries(cacheFileDecodeModes)) {
        ...
        if (typeof decoder.major == "undefined") {
            continue;
        }
        ...
        minorloop: for (let i = minorindex.length - 1; i >= 0; i--) {
            ...
            if (typeof decoder.minor == "number" && decoder.minor != entry.minor) {
                continue;
            }
            try {
