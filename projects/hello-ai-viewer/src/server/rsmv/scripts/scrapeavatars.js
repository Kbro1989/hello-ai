...
async function getPlayerNames(cat, subcat, page) {
    ...
    if (!res.ok) {
        throw new Error(`getplayernames fetch failed ${res.status}, ${res.statusText}`);
    }
...
export async function scrapePlayerAvatars(output, outdir, source, skip, max, parsed) {
    let scene = null;
    if (parsed) {
        if (!source) {
            throw new Error("need file source when extracting avatar data");
        }
...
async function* fetchPlayerAvatars(skip, max) {
    let count = 0;
    const pagesize = 25;
