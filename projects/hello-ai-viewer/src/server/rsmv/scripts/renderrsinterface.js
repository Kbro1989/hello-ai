...
export class UiRenderContext extends TypedEmitter {
    constructor(source) {
        super();
    ...
    async runClientScriptCallback(compid, cbdata) {
        ...
        let inter = await (this.interpreterprom ?? (this.interpreterprom = prepareClientScript(this.source).then(q => new ClientScriptInterpreter(q, this))));
...
export async function loadRsInterfaceData(ctx, id) {
...
export async function renderRsInterfaceHTML(ctx, id) {
...
export function renderRsInterfaceDOM(ctx, data) {
    ...
    updateDom() {
        ...
        if (this.data.modeldata) {
            ...
            if (!isplaceholder && this.ctx.renderer && this.ctx.sceneCache) {
                this.modelrenderer ?? (this.modelrenderer = uiModelRenderer(this.ctx.renderer, this.ctx.sceneCache, this.data.modeldata.positiondata));
