...
export async function extractCacheFiles(output, outdir, source, args, decoderflags) {
    let modeconstr = cacheFileDecodeModes[args.mode];
    if (!modeconstr) {
        throw new Error("unknown mode");
    }
    let flags = { ...decoderflags };
    if (args.batched || args.batchlimit != -1) {
        flags.batched = "true";
    }
        ...
        let flushbatch = () => {
            if (currentBatch) {
                ...
                return (async () => {
                    ...
                    if (batchMaxFiles != -1) {
                        filename += "." + currentBatch.batchchunknr;
                    }
                    filename += `.${mode.ext}`;
    ...
    if (args.edit) {
        ...
        await new Promise(d => process.stdin.once('data', d));
        let lastarchive = null;
        let archedited = () => {
            if (!(source instanceof GameCacheLoader)) {
                throw new Error("can only do this on file source of type gamecacheloader");
            }
            if (lastarchive) {
...
export async function writeCacheFiles(output, source, inputdir, inputfiles) {
    let cachedmodes = {};
    let incompletearchs = new Map();
    let getmode = async (str) => {
        let mode = cachedmodes[str];
        if (!mode) {
            let modecontr = cacheFileDecodeModes[str];
            if (!modecontr) {
                throw new Error(`cache decode mode "${str}" not recognized`);
            }
            mode = cacheFileDecodeModes[str]({});
            cachedmodes[str] = mode;
            await mode.prepareWrite(source);
        }
        return mode;
    };
    let getarch = (major, minor, mode, fetchsiblings = mode.usesArchieves) => {
        let majormap = getOrInsert(incompletearchs, major, () => new Map());
        let group = getOrInsert(majormap, minor, () => ({ fetchsiblings, files: [] }));
        return group;
    };
    let processfile = async (filename, file) => {
        ...
        if (batchjson) {
            ...
            let raw = JSON.parse(file.toString("utf-8"));
            if (!mode.parser) {
                throw new Error(`batch files only supported for json based modes, mode ${batchjson[2]} does not have a json parser`);
            }
    ...
    let processdir = async (inputdir, node) => {
        ...
        for (let file of files) {
            ...
            if (file.kind == "file") {
                await processfile(filename, await inputdir.readFileBuffer(filename));
            }
            if (file.kind == "directory") {
                await processdir(inputdir, filename);
            }
        }
    };
    if (inputdir) {
