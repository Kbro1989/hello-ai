...
import { arrayEnum, trickleTasksTwoStep } from "../utils";
import { iterateConfigFiles } from "../3d/modeltothree";
...
const locationDeps = async (cache, addDep, addHash) => {
    if (cache.classicData) {
        for (let [id, loc] of cache.classicData.objects.entries()) {
            ...
            let hash = crc32(toArrayBufferView(JSON.stringify(loc)));
        ...
        for await (let { id, file } of iterateConfigFiles(cache, cacheMajors.objects)) {
            addHash("loc", id, crc32(toArrayBufferView(file)), 0);
...
const itemDeps = async (cache, addDep, addHash) => {
    if (cache.classicData) {
        for (let [id, item] of cache.classicData.items.entries()) {
            let hash = crc32(toArrayBufferView(JSON.stringify(item)));
            addHash("item", id, hash, 0);
        }
    }
    else {
        for await (let { id, file } of iterateConfigFiles(cache, cacheMajors.items)) {
            addHash("item", id, crc32(toArrayBufferView(file)), 0);
...
const animgroupDeps = async (cache, addDep, addHash) => {
    ...
    for (let file of animgroupfiles) {
        addHash("animgroup", file.fileid, crc32(toArrayBufferView(file.buffer)), 0);
...
const materialDeps = async (cache, addDep, addHash) => {
    ...
    for (let index of indices) {
        ...
        let arch = await cache.getFileArchive(index);
        for (let file of arch) {
            addHash("material", file.fileid, crc32(toArrayBufferView(file.buffer)), index.version);
...
const materialDeps2 = async (cache, addDep, addHash) => {
    ...
    if (cache.getBuildNr() <= lastLegacyBuildnr) {
        ...
        for (let mat of arch) {
            addHash("material", mat.fileid, crc32(toArrayBufferView(mat.buffer)), 0);
...
const npcDeps = async (cache, addDep, addHash) => {
    if (cache.classicData) {
        for (let [id, npc] of cache.classicData.npcs.entries()) {
            let hash = crc32(toArrayBufferView(JSON.stringify(npc)));
            addHash("npc", id, hash, 0);
        }
    }
    else {
        for await (let { id, file } of iterateConfigFiles(cache, cacheMajors.npcs)) {
            addHash("npc", id, crc32(toArrayBufferView(file)), 0);
...
const framesetDeps = async (cache, addDep, addHash) => {
    ...
    for (let index of framesetindices) {
        ...
        addHash("frameset", index.minor, crc32(toArrayBufferView(file.buffer)), index.version);
        let arch = await cache.getFileArchive(index);
        if (arch.length != 0) {
...
export async function getDependencies(cache, args) {
