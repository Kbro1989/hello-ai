...
function defaultAction(major) {
    let majorname = Object.entries(cacheMajors).find(q => q[1] == major)?.[0] ?? `unk_${major}`;
    let r = { name: majorname, comparesubfiles: false, parser: null, outputType: "bin", getFileName: standardName };
    return r;
}
class Loadable {
    constructor(source, major, minor, crc) {
        this.source = source;
        this.major = major;
        this.minor = minor;
        this.crc = crc;
...
export class FileEdit {
    constructor(action, type, major, minor, subfile, before, after) {
        this.action = action;
        this.type = type;
        this.major = major;
        this.minor = minor;
    ...
    async getBefore() {
        try {
            if (this.before == null) {
                throw new Error("no before file");
            }
            if (Buffer.isBuffer(this.before)) {
                return this.before;
            }
    ...
    async getAfter() {
        try {
            if (this.after == null) {
                throw new Error("no after file");
            }
            if (Buffer.isBuffer(this.after)) {
                return this.after;
            }
...
export async function compareCacheMajors(output, sourcea, sourceb, major, minorstart = 0, minorend = 1 << 30) {
...
export async function diffCaches(output, outdir, sourcea, sourceb, ranges) {
    let majors = [];
    ...
    for (let i = 0; i < rootmaxlen; i++) {
        if (ranges.length != 0 && !ranges.some(q => q.end[0] >= i && q.start[0] <= i)) {
            continue;
        }
        if (roota[i] && !rootb[i]) {
            output.log(`major ${i} removed`);
        }
        if (!roota[i] && rootb[i]) {
            output.log(`major ${i} added`);
        }
        if (roota[i] && rootb[i]) {
            majors.push(i);
        }
    ...
    for (let major of majors) {
        ...
        for (let change of newchanges) {
            ...
            let addfile = async (ext, isafter, data) => {
