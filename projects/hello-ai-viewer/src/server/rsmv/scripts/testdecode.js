...
export async function testDecodeHistoric(output, outdir, basecache, before = "", maxchecks = 0) {
    const maxerrs = 50;
                        ...
                        if (!res.success) {
                            ...
                            if (errorcount < maxerrs) {
                                ...
                                if (prevcache && before) {
                                    ...
                                    errfilechunks.push(prevres.getDebugFile("hextext"));
                                }
...
export async function testDecode(output, outdir, source, mode, ranges, opts) {
    ...
    fileiter = async function* () {
        ...
        for (let file of files) {
            ...
            let entry = { major: index.major, minor: index.minor, subfile: file.subindex, file: subfile.buffer };
    ...
    function testFile(file) {
        if (skipMinorAfterError && errminors.indexOf(file.minor) != -1) {
            return true;
        }
        if (skipFilesizeAfterError && errfilesizes.indexOf(file.file.byteLength) != -1) {
            return true;
        }
...
export function testDecodeFile(decoder, buffer, source, args) {
    getDebug(true);
    let error = null;
    let success = false;
    let res = null;
    ...
    let getDebugFile = function (outmode) {
        let errorfile = "";
        if (outmode == "original") {
            errorfile = buffer;
        }
        if (outmode == "json" || outmode == "hextext") {
            ...
            let remainderchunk = { offset: index, len: remainingbytes, label: `remainder: ${remainingbytes}` };
