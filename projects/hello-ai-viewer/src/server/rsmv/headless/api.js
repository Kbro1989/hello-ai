...
import { itemToModel, npcToModel, RSModel } from "../3d/modelnodes";
...
export async function runServer(source, endpoint, auth) {
    let backoff = 1;
    while (true) {
        let res = false;
        try {
...
function runConnection(source, endpoint, auth) {
...
export function getRenderer(width, height, extraopts) {
    let opts = Object.assign({ antialias: true, alpha: true }, extraopts);
    let cnv;
    let ctx = undefined;
    if (typeof HTMLCanvasElement != "undefined") {
        ...
        // Removed __non_webpack_require__("gl") as it's Node.js specific
        cnv = {
            width, height,
        ...
        // ctx = __non_webpack_require__("gl")(width, height, opts); // Removed
    }
    let render = new ThreeJsRenderer(cnv, { context: ctx, ...opts });
    return render;
}
export async function renderAppearance(scene, mode, argument, headmodel = false) {
    let width = 500;
    let height = 700;
    ...
    if (mode == "player" || mode == "appearance") {
        ...
        if (mode == "player") {
            ...
            if (appearance.indexOf("404 - Page not found") != -1) {
