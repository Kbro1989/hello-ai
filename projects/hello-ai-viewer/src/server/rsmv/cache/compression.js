...
import * as pako from "pako"; // Import pako
import * as bzip2 from "../libs/bzip2fork"; // Change require to import
import { LZMA } from "lzma/src/lzma_worker.js"; // Change require to import
//decompress data as it comes from the server
export function decompress(input, key) {
...
export function compressSqlite(input, compression) {
    switch (compression) {
        case "zlib":
...
export function legacybz2(input) {
...
var _zlib = function (input, key) {
    try {
        ...
        if (key) {
            ...
            return Buffer.from(pako.ungzip(compressedData.slice(4, 4 + compressedsize)));
        }
        else {
            let compressedData = input.slice(9, 9 + compressedsize);
            return Buffer.from(pako.ungzip(compressedData));
        }
    }
    catch (e) {
...
export function legacyGzip(input) {
    return Buffer.from(pako.ungzip(input));
}
// Removed nativelzma and __non_webpack_require__ block
...
function _zlibSqlite(input) {
    ...
    return Buffer.from(pako.inflate(input.slice(0x8)));
}
function _zlibSqliteCompress(input) {
    let compressbytes = Buffer.from(pako.deflate(input));
