...
export function packSqliteBufferArchive(buffers) {
    return new Archive(buffers).packSqlite();
}
export function unpackSqliteBufferArchive(buffer, subids, namehashes) {
    if (subids.length == 1) {
...
export function packBufferArchive(buffers) {
    return new Archive(buffers).packNetwork();
}
export function unpackBufferArchive(buffer, subids, namehashes) {
    if (subids.length == 1) {
        let r = [{
                buffer: buffer,
                offset: 0,
...
export function rootIndexBufferToObject(metaindex, source) {
...
export function indexBufferToObject(major, buffer, source) {
...
export function fileIdToArchiveminor(major, fileid, buildnr) {
    if (buildnr < 488) {
...
export function archiveToFileId(major, minor, subfile) {
    let archsize = mappedFileIds[major] ?? 1;
...
export class DirectCacheFileSource extends CacheFileSource {
