...
const handshake1 = new FileParser(["struct",
    ["type", "ubyte"],
    ["length", "ubyte"],
    ["version1", "uint"],
    ["version2", "uint"],
...
const handshake2 = new FileParser(["struct",
    ["op", "ubyte"],
...
const filereq1 = new FileParser(["struct",
    ["mode", "ubyte"],
    ["major", "ubyte"],
    ["minor", "uint"],
    ["version", "ushort"],
...
export function parseClientConfig(cnf) {
...
export async function downloadServerConfig() {
    let body = await fetch("http://world3.runescape.com/jav_config.ws?binaryType=2").then(r => r.text());
