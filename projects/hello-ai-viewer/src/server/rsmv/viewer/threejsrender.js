...
import { BufferGeometry, Clock, Group, Mesh, PerspectiveCamera, Texture, Vector3 } from "three";
import { VR360Render } from "./vr360camera";
import { SkewOrthographicCamera } from "../map";
import { updateItemCamera } from "./scenenodes";
//TODO remove
...
export class ThreeJsRenderer extends TypedEmitter {
    constructor(canvas, params) {
        ...
        this.itemcam = new THREE.PerspectiveCamera();
        this.guaranteeGlCalls = this.guaranteeGlCalls.bind(this);
        this.render = this.render.bind(this);
        this.forceFrame = this.forceFrame.bind(this);
...
export function disposeThreeTree(node) {
    if (!node) {
        return;
    }
...
export async function exportThreeJsGltf(node) {
    ...
    let res = await new Promise((resolve, reject) => {
        exporter.parse(node, gltf => resolve(gltf), {
            binary: true,
            animations: anims
        });
    });
...
export function exportThreeJsStl(node) {
...
export function highlightModelGroup(vertexgroups) {
