...
import { Map2dView } from "../map/map2d";type LookupMode = "model" | "item" | "npc" | "object" | "material" | "map" | "avatar" | "spotanim" | "scenario" | "scripts";
...
function ScenePlayer(p: LookupModeProps) {
	...
	type PlayerId = { player: string, head: boolean, data: Buffer };
	const typedId = id as PlayerId | undefined;
	const player = typedId?.player ?? (p.initialId && typeof p.initialId == "object" && typeof (p.initialId as any).player == "string" ? (p.initialId as any).player : "");
	const head = typedId?.head ?? (p.initialId && typeof p.initialId == "object" && typeof (p.initialId as any).head == "boolean" ? (p.initialId as any).head : false);

	const oncheck = (e: React.ChangeEvent<HTMLInputElement>) => {
		if (typedId) { setId({ player: typedId.player, data: typedId.data, head: e.currentTarget.checked }); }
	}
