...
export function downloadBlob(name: string, data: any) {
				...
				if (item.getAsFileSystemHandle) {
					...
					if (filehandle.kind == "file") {
						...
						for await (let handle of dir.values()) {
							if (handle.kind == "file") {
								files[handle.name] = await (handle as FileSystemFileHandle).getFile();
							}
						}
					}
	...
	return (
		<div className="mv-hexrow">
			<div>
				...
				<input type="button" className="sub-btn" onClick={e => downloadBlob("file.bin", new Blob([toBlobPart(buffer)], { type: "application/octet-stream" }))} value="download original" />
	...
	return (
		<div style={{ display: "grid", gridTemplateRows: "auto 1fr" }}>
			<div className="mv-modal-head">
				...
				<span style={{ float: "right", marginLeft: "10px" }} onClick={e => downloadBlob(p.file.name, new Blob([toBlobPart(p.file.data)]))}>download</span>
