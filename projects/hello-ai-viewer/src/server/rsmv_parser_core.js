...
var debugdata = null;
export function getDebug(trigger) {
    let ret = debugdata;
...
export function buildParser(parent, chunkdef, typedef) {
...
export function buildReference(name, container, startingpoint) {
    if (!container) {
...
const hardcodes = {
    ...
    buildnr: function (args, parent, typedef) {
        return {
            readConst(state) {
...
`
    ...
    scriptopt: function (args, parent, typedef) {
        return {
            read(state) {
                let cali = state.args.clientScriptDeob; // ClientscriptObfuscation
                if (!cali) {
