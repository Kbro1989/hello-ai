...
import { worldStride, CombinedTileGrid, classicChunkSize, rs2ChunkSize, tiledimensions, getTileHeight, parseMapsquare } from "../3d/mapsquare";
...
import { EngineCache, ThreejsSceneCache } from "../modeltothree";
import { delay, stringToFileRange, trickleTasks } from "../utils";
import { drawCollision } from "./collisionimage";
import prettyJson from "json-stringify-pretty-compact";
import { chunkSummary, compareFloorDependencies, compareLocDependencies, mapsquareFloorDependencies, mapsquareLocDependencies, pointsIntersectProjection, RenderDepsTracker } from "./chunksummary";
...
export async function purgeBadRenders(config, versionfilter) {
...
export async function runMapRender(output, filesource, config, forceCheck) {
...
export class MapRenderer {
    constructor(cnv, config, engine, deps, opts) {
        ...
        cv.addEventListener("webglcontextlost", async () => {
            let isrestored = await new Promise((done, err) => {
                let cleanup = (v) => {
                    cv.removeEventListener("webglcontextrestored", handler);
                    clearTimeout(timer);
                    done(v);
                };
                let handler = () => cleanup(true);
                cv.addEventListener("webglcontextrestored", handler);
...
export async function downloadMap(output, getRenderer, engine, deps, rects, config, progress) {
    let errs = [];
    const zscan = 4;
    const maxretries = 1;
    let chunks = [];
        ...
        for (let chunk of chunks) {
            ...
            let fn = (async () => {
                ...
                for (let retry = 0; retry <= maxretries; retry++) {
                    try {
                        let renderprom = lastrender.then(() => maprender ?? (maprender = getRenderer()));
...
class SimpleHasher {
    ...
    rectexists(rect) {
        ...
        for (let z = rect.z; z < rect.z + rect.zsize; z++) {
            for (let x = rect.x; x < rect.x + rect.xsize; x++) {
                exists || (exists = this.depstracker.deps.hasEntry("mapsquare", x + z * worldStride));
            }
        }
        return exists;
    }
...
export function renderMapsquare(engine, config, depstracker, mipper, progress, chunkx, chunkz) {
                        ...
                        findparent: for (let parentoption of parentCandidates) {
                            for (let versionMatch of await parentinfo.findMatches(this.datarect, parentoption.name)) {
                                ...
                                for (let chunk of chunks) {
                                    ...
                                    let modelmatrix = new Matrix4().makeTranslation(chunk.model.chunkx * tiledimensions * chunk.model.loaded.chunkSize, 0, chunk.model.chunkz * tiledimensions * chunk.model.loaded.chunkSize).premultiply(chunk.model.rootnode.matrixWorld);
                                    ...
                                    // continue optloop;
                                    // }
                                    isdirty || (isdirty = pointsIntersectProjection(proj, locs));
                                    isdirty || (isdirty = pointsIntersectProjection(proj, floor));
                                    if (isdirty) {
                                        break;
                                    }
                                }
...
export function mapImageCamera(x, z, ntiles, dxdy, dzdy) {
    let scale = 2 / ntiles;
    let cam = new Camera();
